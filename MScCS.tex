...
\theabstract{
Many people suffering from kidney failure are in need of a kidney transplantation. A problem in the health care is that these patients cannot undergo surgery if they have too much belly fat. Regular exercise and physical activity are therefore crucial for this group of people. A project group at the Skåne University Hospital has recently been established to help patients with kidney failure to lose weight. A question they asked themselves was whether it was possible to use activity tracking devices to monitor patient’s activities. 

The purpose of this thesis was to design and evaluate a system that can be used to track the physical activities of the patients. The system was built using a Sony SmartBand, a wristband collecting and analyzing data about a user's daily activities. The conclusions of the thesis is that the system can be built but becomes complicated to use due to Sony’s API.
}
...
%% Förslag på ändring
% RSAT was developed to be integrated in a planning system already in use at the hospital. The planning system is develop in a research project called itACiH. itACiH investigates the possibilities of advanced health care at home. The planning system is built as a web application and can fetch data from sensors that are installed in a patient's home. 
...
%The purpose of this thesis is to build a system that can be used in the health care to help patients with kidney failure to loose weight.
...
When the clinician gives the smart band and the phone to the patient they only need to link the patients profile to the phone in RSAT. %%Denna meningen är urusel 
