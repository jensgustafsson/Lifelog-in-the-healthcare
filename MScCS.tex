...
So when the clinician gives smart band and phone to the patient the only thing the clinicians do is linking the patients profile to the phone in RSAT. %%Denna meningen är urusel 
...
\begin{itemize}
\item \emph{The database goal service}, which are used to store all goals of a user. When the clinician inputs a goal for an activity in to RSAT this service save that goal to the database. The service also handles the fetching of these goals. %(TODO: skriva mer om) kolla jens.
...
%The Sony Lifelog platform is user-focused. This means that each user has their own set of devices that tracks data about them. This is not what the RSAT-group wants. What the RSAT-group wants is quite the opposite, they want a system that is device focused, where each device is connected to a user and not the other way around. 

%To conclude the use of the Lifelog platform we will discuss the pros and cons of the platform form the use in RSAT. The API was in our opinion easy to use. A simple REST API and good documentation made it easy to see the possibilities with the platform. Getting the activity data was quit strait forward. No to the cons of Lifelog and SWR10, a limitation of SWR10 is that the only way to get activity data from SWR10 is thought the Lifelong API. We ware not able to get the data directly on the phone without gonging thought the Lifelog cloud API. 

%The obvious limitation of not being able to read goal from the API was annoying and forced us of making a clumsy workaround that force the patient and clinician input the goals both in RSAT and the Lifelog app.  There are two possible solutions for this problem. One is the possibility of writhing an app for the patient to use and that way to set goals. The other possibility is for the Lifelog api to change for allowing goals to be read and writhed.

%If the API was writable it could be possible for the clinician send feedback to the Lifelog app running on the patients phone. It would also be possible for the clinician to change the goals of the patient without the need for the patient do visit the clinician. 

%The user focus approached of the Lifelog API force us to make an Google or Sony Account for every patent that want to use the system. A better approach in our use case is a device focus approach. Where every divice can be manege by one login and the system only needed one account and connect all devises to that account. To get the data from a device only specify that device and get the data from the API. When a device change patent it can be done using only a one click action in RSAT if sony had a more device focused approach.
